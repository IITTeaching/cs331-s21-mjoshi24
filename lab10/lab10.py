...
class AVLTree:
    ...
    def rebalance(t):
        ...
        nb = rebhelper(t)
        if nb < 0:
          if rebhelper(t.right) > 0:
            t.right.rotate_right()
          t.rotate_left()
          AVLTree.rebalance(t.left)
        elif nb > 0:
          if rebhelper(t.left) < 0:
            t.left.rotate_left()
          t.rotate_right()
    ...
    def __delitem__(self, val):
        assert(val in self)
        ### BEGIN SOLUTION
        def delHelper(node, key):
            if not node:
                return node
            elif key < node.val:
                node.left = delHelper(node.left, key)
            elif key > node.val:
                ...
                while min.left:
                    min = min.left
                node.val = min.val
                node.right = delHelper(node.right, min.val)
            AVLTree.rebalance(node)
            return node
        self.root = delHelper(self.root, val)
        self.size -= 1
        ### END SOLUTION
