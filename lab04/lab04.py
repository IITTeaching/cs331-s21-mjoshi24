...
class ArrayList:
    ...
    def append(self, value):
        ...
        ### BEGIN SOLUTION
        copynum = 2 * self.len + 1
        if self.len == len(self.data): # full
            lst = ConstrainedList(copynum) 
    ...
    def pop(self, idx=-1):
        ...
        num = self._normalize_idx(idx)
        val = self.data[num]
        for i in range(num + 1, self.len):
            self.data[i-1] = self.data[i] #back
        self.len -= 1 
